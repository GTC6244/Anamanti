...
//! file** (`ambient.json` in the working directory by default, overridable with
...
pub const DEFAULT_CONFIG_FILE: &str = "ambient.json";
...
pub struct Config {
    ...
    /// `instance_id` in its `ambient.json`, so it never falls through to the branch.
    ...
    /// to `ambient_settings.json`. This is a **separate** file from the boot config:
...
/// `ambient.json`). Used as the default `instance_id` so a copy launched from a test
...
impl Config {
    ...
    /// `ambient.json` in the working directory is used when present; when absent, the
    ...
    pub fn shared_settings(&self) -> Result<Arc<SharedSettings>> {
        ...
        if let Some(p) = persist_path.as_deref().and_then(load_persisted) {
            ...
            // ambient.json still takes effect after an older file is loaded.
...
mod tests {
    ...
    fn shipped_example_config_is_valid() {
        // The committed `ambient.example.json` must always parse under
        // `deny_unknown_fields` and resolve, so it can't drift out of sync with the
        // schema.
        let example = concat!(env!("CARGO_MANIFEST_DIR"), "/ambient.example.json");
        let c = Config::load(Some(std::path::Path::new(example)))
            .expect("ambient.example.json must be a valid config");
        assert_eq!(c.service_name, "Ambient Orchestrator");
        assert_eq!(c.llm_label(), "ollama");
    }
