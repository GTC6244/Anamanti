# Ambient music routing — deployment runbook

House-wide music from the Mac orchestrator host to Snapcast speakers. Design:
...
Mac (hub)                                              Speakers
  ...
  orchestrator ──JSON-RPC :1705 (duck/select)┘        (Linux main, rooms, Echo Show*)
```
...
- **Web:** `echo '{"command":["loadfile","https://stream-url"]}' | socat - /tmp/ambient-mpv.sock`
  (or let the orchestrator's future `url_play` tool do it) → audio on the speakers.
...
and never affects a turn. Implementation: `orchestrator/src/music/` (unit-tested
...
- P4 (ducking): **implemented & tested** in `orchestrator/src/music/`.
