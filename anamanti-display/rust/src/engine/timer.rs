...
//!
//! A tool the LLM calls on the Mac emits an `anamanti-timer` frame during the voice
...
/// `anamanti-speak` frame, then streams the `audio-start`/`audio-chunk`…/`audio-stop`
