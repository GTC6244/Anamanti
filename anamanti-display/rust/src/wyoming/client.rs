    ...
    W: AsyncWrite + Unpin,
{
    ...
    /// Send an `anamanti-interrupt` (barge-in) frame so the orchestrator aborts the
...
mod tests {
    ...
    async fn follow_up_listen_frame_surfaces_before_the_turn_ends() {
        ...
        // Mock orchestrator: transcript → reply → TTS audio, then an `anamanti-listen`
    ...
    async fn barge_in_interrupt_ends_the_turn() {
        ...
        assert_eq!(updates.last(), Some(&TurnUpdate::Finished));
        drop(conn);
        // The device relayed an `anamanti-interrupt` frame so the orchestrator can
