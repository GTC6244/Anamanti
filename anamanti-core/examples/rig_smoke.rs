...
//!   ollama pull qwen2.5     # or set ANAMANTI_OLLAMA_MODEL to one you have
//!
...
//!   ANAMANTI_WEB_SEARCH=1 cargo run --no-default-features --features rig \
//!       --example rig_smoke -- "what happened in the news today?"
//!
//! Env:
//!   ANAMANTI_OLLAMA_URL    default http://127.0.0.1:11434
//!   ANAMANTI_OLLAMA_MODEL  default qwen2.5
//!   ANAMANTI_WEB_SEARCH=1  enable the internet_search tool (real web request)
...
use anamanti_core::directions::LiveHomeLocation;
use anamanti_core::llm::rig::{tools_from_config, RigBackend, Tools};
use anamanti_core::llm::{LlmBackend, LlmTurn};
...
async fn main() -> anyhow::Result<()> {
    ...
    let web_search = matches!(
        std::env::var("ANAMANTI_WEB_SEARCH")
            .unwrap_or_default()
            .to_lowercase()
            .as_str(),
        "1" | "true" | "on" | "yes"
    );
    let backend_kind = std::env::var("ANAMANTI_LLM_BACKEND")
    ...
    let backend: Arc<dyn LlmBackend> = match backend_kind.as_str() {
        "anthropic" | "claude" => {
            let key = std::env::var("ANTHROPIC_API_KEY").map_err(|_| {
                anyhow::anyhow!("ANAMANTI_LLM_BACKEND=anthropic needs ANTHROPIC_API_KEY")
            })?;
            let model = std::env::var("ANAMANTI_ANTHROPIC_MODEL").map_err(|_| {
                anyhow::anyhow!(
                    "set ANAMANTI_ANTHROPIC_MODEL to a valid model id (e.g. a current Claude model)"
                )
            })?;
            let base = std::env::var("ANAMANTI_ANTHROPIC_BASE_URL")
                .unwrap_or_else(|_| "https://api.anthropic.com".into());
            let max_tokens: u32 = std::env::var("ANAMANTI_ANTHROPIC_MAX_TOKENS")
                .ok()
        ...
        _ => {
            let url = std::env::var("ANAMANTI_OLLAMA_URL")
                .unwrap_or_else(|_| "http://127.0.0.1:11434".into());
            let model = std::env::var("ANAMANTI_OLLAMA_MODEL").unwrap_or_else(|_| "qwen2.5".into());
